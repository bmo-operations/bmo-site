...
export default function RosterGrid({ year, players, onPopupPlayer }: { year: number, players: Player[], onPopupPlayer: (p: Player) => void }) {

    const rosterCards = players.map(p => (<RosterCard key={p.id} player={p} year={year} onMore={() => onPopupPlayer(p)} />))
    return (
